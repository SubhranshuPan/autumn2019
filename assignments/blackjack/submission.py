...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state, action, reward, newState):
        ...
        # current prediction Q̂(s,a)
        prediction = self.getQ(state, action)

        # Compute target = r + γ · max_{a'} Q̂(s', a')
        if newState is None:
            target = reward
        else:
...
def compare_changed_MDP(original_mdp, modified_mdp, featureExtractor):
    ...
    print(f"[compare_changed_MDP] Original policy on modified MDP: avg reward = {avg_fixed:.4f}")

    # Step 3: Q-learning directly on modified MDP
    rl = QLearningAlgorithm(modified_mdp.actions, modified_mdp.discount(), featureExtractor, explorationProb=0.2)
    simulate(modified_mdp, rl, numTrials=30000, maxIterations=1000)
    rl.explorationProb = 0
    greedy_policy = {}
    for s in modified_mdp.states:
        if modified_mdp.actions(s):
            greedy_policy[s] = max((rl.getQ(s, a), a) for a in modified_mdp.actions(s))[1]
    ...
    print(f"[compare_changed_MDP] Q-learning on modified MDP:    avg reward = {avg_ql:.4f}")
    # END_YOUR_CODE
